...
with h5py.File("dataset.h5", "w") as f:
    ...
    for i in range(0, 200000, 1000):
        dataset_x[i:i+1000] = np.random.randint(0, 256, size=(1000, 128, 128, 3), dtype="uint8")
        dataset_y[i:i+1000] = np.random.randint(0, 10, size=(1000,), dtype="uint8")
...
# dataset.h5 fait plus de 9 Go mais en RAM on utilise moins de 500 Mo pour faire ces opérations
...
with h5py.File("dataset.h5", "r") as f:
    ...
    print(f["x_train"][1000:1500]) # On ne chargera en mémoire vive qu'un chunk de 1000 exemples
    print(f["x_train"][1000:2500]) # On ne chargera en mémoire vive que deux chunks de 1000 exemples
```
